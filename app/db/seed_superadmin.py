...
async def get_or_create_tenant(db) -> Tenant:
    result = await db.execute(select(Tenant).where(Tenant.ruc == "1790000000001"))
...
async def ensure_super_admin(db, tenant_id: int) -> None:
    email = "super_admin@facsys.com"
    ...
    db.add(super_admin)
    await db.commit()
    print(f"Usuario Super Admin creado asignado al tenant_id '{tenant_id}'")
...
async def run_seed():
    async for db in get_db_session():
        try:
            ...
            await ensure_super_admin(db, tenant.id)
